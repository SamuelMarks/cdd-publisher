...
pub fn publish_npm_with_exe(dir: &Path, token: &str, exe: &str) -> Result<()> {
    ...
    fs::write(&npmrc_path, format!("//registry.npmjs.org/:_authToken={token}"))?;
...
mod tests {
    ...
    fn create_dummy_exe(dir: &Path, name: &str, exit_code: i32, expected_args: Option<&str>) -> std::path::PathBuf {
...
"
        ...
        fs::set_permissions(&exe_path, fs::Permissions::from_mode(0o755)).expect("Failed to set perms");
        exe_path
    }
    ...
    fn test_publish_npm_success() {
        ...
        let result = publish_npm_with_exe(dest.path(), "secret123", exe.to_str().expect("Valid path"));
        assert!(result.is_ok());
    ...
    fn test_publish_pypi_success() {
        ...
        let result = publish_pypi_with_exe(dest.path(), "secret123", exe.to_str().expect("Valid path"));
        assert!(result.is_ok());    }
    ...
    fn test_publish_cargo_success() {
        ...
        let result = publish_cargo_with_exe(dest.path(), "secret123", exe.to_str().expect("Valid path"));
        assert!(result.is_ok());    }
