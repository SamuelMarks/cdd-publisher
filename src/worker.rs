...
use crate::publish::{publish_cargo, publish_npm, publish_pypi};
...
impl EventQueue for redis::aio::MultiplexedConnection {
    ...
    async fn read_one(&mut self, stream: &str, group: &str, consumer: &str) -> Result<Option<(String, Vec<u8>)>> {
        ...
        #[cfg(not(tarpaulin_include))]
        let reply: redis::streams::StreamReadReply = redis::AsyncCommands::xread_options(self, &[stream], &[">"], &opts).await?;
        ...
        let payload_bytes: Vec<u8> = redis::FromRedisValue::from_redis_value(payload_value.clone()).unwrap_or_default();
...
impl<Q: EventQueue> Worker<Q> {
    ...
    pub async fn run_once(&mut self) -> Result<()> {
        let msg = self.connection.read_one(&self.stream_name, &self.group_name, &self.consumer_name).await?;
    ...
    async fn acknowledge(&mut self, id: &str) -> Result<()> {
        #[cfg(not(tarpaulin_include))]
        self.connection.ack(&self.stream_name, &self.group_name, id).await?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_publish_job_serialization() {
        ...
        if let Ok(json) = serde_json::to_string(&job) {
            assert_eq!(json, r#"{"artifact_id":"test-artifact-123","registry":"npm"}"#);
    ...
    async fn test_worker_new_success() {
        ...
        let worker = Worker::new(mock_queue, "stream", "group", "consumer", RegistryTokens::default(), get_dummy_audit_client()).await;
        assert!(worker.is_ok());    }
    ...
    async fn test_worker_run_once_no_messages() {
        ...
        let mut worker = Worker::new(mock_queue, "stream", "group", "consumer", RegistryTokens::default(), get_dummy_audit_client()).await.expect("Failed to create");
    ...
    async fn test_worker_run_once_empty_payload() {
        let mut mock_queue = MockRedisQueue::new();
        mock_queue.expect_create_group().returning(|_, _| Ok(()));
        mock_queue.expect_read_one().returning(|_, _, _| Ok(Some(("1-0".to_string(), Vec::new()))));
    ...
    async fn test_worker_run_once_invalid_json() {
        let mut mock_queue = MockRedisQueue::new();
        mock_queue.expect_create_group().returning(|_, _| Ok(()));
        mock_queue.expect_read_one().returning(|_, _, _| Ok(Some(("1-0".to_string(), b"invalid".to_vec()))));
        mock_queue.expect_ack().returning(|_, _, _| Ok(()));
    ...
    async fn test_worker_process_job() {
        ...
        let tokens = RegistryTokens { npm: Some("token".into()), pypi: Some("token".into()), cargo: Some("token".into()) };
        let mut worker = Worker::new(mock_queue, "stream", "group", "consumer", tokens, get_dummy_audit_client()).await.expect("Failed to create");
        ...
        mock_queue_no_pypi.expect_create_group().returning(|_, _| Ok(()));
        let mut worker_no_pypi = Worker::new(mock_queue_no_pypi, "stream", "group", "consumer", RegistryTokens::default(), get_dummy_audit_client()).await.expect("Failed to create");
        ...
        mock_queue_no_cargo.expect_create_group().returning(|_, _| Ok(()));
        let mut worker_no_cargo = Worker::new(mock_queue_no_cargo, "stream", "group", "consumer", RegistryTokens::default(), get_dummy_audit_client()).await.expect("Failed to create");
    ...
    async fn test_worker_run_once_process_error() {
        ...
        mock_queue_err.expect_create_group().returning(|_, _| Ok(()));
        let valid_json_unsupported = br#"{"artifact_id":"123","registry":"unsupported"}"#.to_vec();
        mock_queue_err.expect_read_one().returning(move |_, _, _| Ok(Some(("1-0".to_string(), valid_json_unsupported.clone()))));
        // Expect NO ack since processing fails
        let mut worker_err = Worker::new(mock_queue_err, "stream", "group", "consumer", RegistryTokens::default(), get_dummy_audit_client()).await.expect("Failed to create");
    ...
    async fn test_worker_run_once_success() {
        ...
        let mut worker = Worker::new(mock_queue, "stream", "group", "consumer", tokens, get_dummy_audit_client()).await.expect("Failed to create worker");
